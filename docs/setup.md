...
| `ALLOWED_EMAILS` | Optional. Comma-separated allowlist of addresses that may sign in, case insensitive. Unset, anyone who reaches your public URL can request a magic link and gets their own workspace, which is worth closing on an instance you run for yourself. Teammates you invite from the workspace settings get the invitation by email and can sign in without being listed, as long as the invitation is pending or they are still a member. |
